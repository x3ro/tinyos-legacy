function output=bless_broadcast(varargin)
...
ws('insert_event', 'bless_broadcast', 'clock_tick', start_time, id, {});
...
if id ~= sim_params.base_station & all_mote.parent(id) == protocol_params.invalid_parent
    % Send routing packet
    packet = bless_broadcast_routing_packet(id, 0, protocol_params.route_packet_length);
...
ws('insert_event', 'bless_broadcast', 'clock_tick', interval, id, {});
...
function bless_broadcast_packet = bless_broadcast_routing_packet(source, hop, length)
global protocol_params radio_params;
bless_broadcast_packet.source = source;
bless_broadcast_packet.hop = hop;
bless_broadcast_packet.type = protocol_params.route_packet_type;
bless_broadcast_packet.length = length;
bless_broadcast_packet.parent = radio_params.broadcast_addr;
...
if rand < prob & all_mote.route_count(id) == 0
    ...
    bless_broadcast_packet = bless_broadcast_routing_packet(id, all_mote.hop(id), length);
    feval(sim_params.protocol_send,'send_packet', id, bless_broadcast_packet);
