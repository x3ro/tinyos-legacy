function output = collect_data(varargin)
...
app_params.data_clock_interval = 20*100000;
...
phase_shift(id) = 0;
ws('insert_event', 'collect_data', 'clock_tick', start_time, id, {});
void = -1;
...
global all_mote sim_params protocol_params app_params phase_shift mote_stat;
...
if id == sim_params.base_station | all_mote.parent(id) ~= protocol_params.invalid_parent
    packet = send_data_packet(id, id, all_mote.parent(id), app_params.packet_length, mote_stat.app_seqnum(id));
    feval(sim_params.protocol, 'send_packet', id, packet);
end

interval = app_params.data_clock_interval;
ws('insert_event', 'collect_data', 'clock_tick', interval + phase_shift(id), id, {});
if (phase_shift(id) ~= 0)
    phase_shift(id) = 0;
end
void = -1;
...
%% PRIVATE FUNCTION
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data_packet = send_data_packet(data_source, source, parent, length, seq_num)
global app_params
...
data_packet.app_seq_num = seq_num;