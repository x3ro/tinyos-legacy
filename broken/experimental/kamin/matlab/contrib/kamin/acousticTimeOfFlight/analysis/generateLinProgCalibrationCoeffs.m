...
for transmitter=1:n
    for receiver=1:n
        for time = 1:t
            if rangingData(transmitter, receiver, time, 1)>0 %& rangingData(transmitter, receiver, time, 1)<=100
                %if the distance between i and j is known at time t,
                %minimize the absolute value of the error, i.e. |f(tij, Ti, Rj)-dij|.  
                %We can do this by introducing a dummy variable
                
                addDummyVariable;%(add Zij)
                f(end)=1; %minimize the dummy variable

                %f(tij, Ti, Rj)-dij <=Zij
                row=row+1;%add inequality
                A(row,end) = -1; 
                A(row, 1:length(x)) = polynomialCalibrationFunction(rangingData(transmitter, receiver,time,2),transmitter,receiver,polynomialDegree, length(x));
...
%                 A(row, 1:length(x)) = polynomialCalibrationFunction(rangingData(transmitter, receiver,time,2),transmitter,receiver,polynomialDegree, length(x));
%                 A(row, 1:length(x)) =A(row, 1:length(x)) - polynomialCalibrationFunction(rangingData(receiver,transmitter, time,2),receiver,transmitter,polynomialDegree, length(x));
%                 A(row,end) = -1; 
%                 b(row) = 0;
% 
