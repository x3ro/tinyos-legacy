function coefficients=generateLinProgCalibrationCoeffs(rangingData, varargin)
%coefficients=generateLinProgCalibrationCoeffs(rangingData, polynomialDegree)
%
...
for transmitter=1:n-1
    for receiver=transmitter+1:n
        for time = 1:t
            if rangingData(transmitter, receiver, time, 1)>=0 %if the distance between i and j is known at time t
                ...
                A(row,end) = -1; 
                A(row, 1:size(x)) = calibrationFunction(rangingData(transmitter, receiver,time,2),transmitter,receiver,polynomialDegree, size(x), size(params));
                b(row) = rangingData(transmitter, receiver,time,1);

                if rangingData(receiver, transmitter, time, 1)>=0 %if the distance between i and j is known at time t
                    addDummyVariable;%(add Zij2)
                    row=row+1;%add inequality
                    %f(tji, Tj, Ri)-dji <=Zij2
                    f(end)=1; %minimize that dummy variable
                    A(row,end) = -1; 
                    A(row, 1:size(x)) = calibrationFunction(rangingData(receiver,transmitter, time,2),receiver,transmitter,polynomialDegree, size(x), size(params));
                    b(row) = rangingData(receiver,transmitter, time,1);
                end
                ...
                %(if i>=j, we already added all of the inequalities)
                    ...
                    f(end)=1; %minimize that dummy variable
                    A(row, 1:size(x)) = calibrationFunction(rangingData(transmitter, receiver,time,2),transmitter,receiver,polynomialDegree, size(x), size(params));
                    A(row, 1:size(x)) =A(row, 1:size(x)) - calibrationFunction(rangingData(receiver,transmitter, time,2),receiver,transmitter,polynomialDegree, size(x), size(params));
                    A(row,end) = -1; 
                    b(row) = 0;

                    %f(tji, Tj, Ri)- f(tij, Ti, Rj) <=Zij
                    f(end)=1; %minimize that dummy variable
                    A(row, 1:size(x)) = -calibrationFunction(rangingData(transmitter, receiver,time,2),transmitter,receiver,polynomialDegree, size(x), size(params));
                    A(row, 1:size(x)) =A(row, 1:size(x)) + calibrationFunction(rangingData(receiver,transmitter, time,2),receiver,transmitter,polynomialDegree, size(x), size(params));
                    A(row,end) = -1; 
