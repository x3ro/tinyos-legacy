 ...
 */
{
  ...
  newtio.c_cc[VEOL]     = _POSIX_VDISABLE;
  ...
  if (tcflush(fd, TCIFLUSH) >= 0 &&
    ...
    {
      int state, rval;
      rval = ioctl(fd, TIOCMGET, &state);
      if (rval < 0) {printf("failed to get termio RTS.\n");}
      state &= ~TIOCM_RTS;
      rval = ioctl(fd, TIOCMSET, &state);
      if (rval < 0) {
        printf("failed to set termio RTS: %s\n", strerror(errno));
      }
      rval = ioctl(fd, TIOCCDTR);
      if (rval < 0) {printf("failed to set termio DTR.\n");}
      rval = tcflush(fd, TCIFLUSH);
      if (rval < 0) {printf("failed to flush termio input.\n");}
      rval = tcflush(fd, TCOFLUSH);
      if (rval < 0) {printf("failed to flush termio output.\n");}
      ...
      if (src)
	{
	  ...
	  src->send.seqno = 37;
	  if (DEBUG) {
	    printf("opened serial port %s with speed %i\n", device, baud_rate); 
          }
	  return src;
	}
    }
