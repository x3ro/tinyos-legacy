...
implementation {
  ...
  void allocateAbort(fileresult_t result) {
    switch (state)
      {
      case s_creating: signal IFileWrite.newDone[writefd](result); break;
  ...
  bool updateMeta() {
    bool updateMeta = fds[writefd].check;
  ...
  command void IFileWrite.new[uint8_t fd](bool check) {
    state = s_creating; 
    writefd = fd;
    fds[fd].offset = 0;
    fds[fd].lastOffset = 0;
	...
	case s_creating:
	  if (--newCount)
	  ...
	  else
	    {
	      ...
	      signal IFileWrite.newDone[writefd](FS_OK);
	    }
	  return;
	case s_allocating2:
  ...
  default event void IFileWrite.newDone[uint8_t id](fileresult_t result) {
    assert(0);
  }
