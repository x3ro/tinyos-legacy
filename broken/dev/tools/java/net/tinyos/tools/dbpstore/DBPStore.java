...
public class DBPStore {
    ...
    static DBWriterMySQL dbwriter;
    private static boolean bSchemaPenthouse = false;
    private static boolean bSchemaPackets = false;

    public DBPStore (String host, String port) {
	...
	while ((i = in.read()) != -1) {
		...
		if (count == MSG_SIZE) {
			...
			dbwriter.WritePacket ( packet );
		    }
		    if ( bSchemaPenthouse ) {
			// store temperature and sound data
			dbwriter.WriteSensorReadings ( packet );
		    }
		    System.out.println();
		    count = 0;
		}
	...
	bSchemaPenthouse = true;
	boolean bSuccess = false;
	System.out.println("\nlisten started");
	dbwriter = new DBWriterMySQL();
	dbwriter.Connect();
	DBPStore reader = new DBPStore(args[0],args[1]);
	try {
	BeaconInjector bi = new BeaconInjector(args[0], Integer.parseInt(args[1]),(byte) 0x42);
