...
char TOS_COMMAND(PACKET_INIT)(){
	MAKE_PW2_OUTPUT();
TOS_CALL_COMMAND(MIC_INIT)();
TOS_CALL_COMMAND(MIC_PWR)(1);
TOS_CALL_COMMAND(MIC_MUX_SEL)(1);
TOS_CALL_COMMAND(MIC_POT_ADJUST)(16);
VAR(sounder_length) = 30;
...
char TOS_EVENT(MIC_SIGNAL_TONE_DETECTED)(void){
  //time stamp the reception of the sound
  TOS_CALL_COMMAND(RF_COMM_LEDy_TOGGLE)();
  VAR(rec_ptr)->tone_time = TOS_CALL_COMMAND(RF_COMM_SUB_GET_CURRENT_TIME)();
  return 1;
};
