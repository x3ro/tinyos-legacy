...
interface SoundBlock {
  command result_t Get(uint8_t *data, uint16_t len, uint8_t gain);
  //command result_t Get(uint16_t *data, uint16_t len, uint8_t gain);
  async event void GetDone();
} // SoundBlock
