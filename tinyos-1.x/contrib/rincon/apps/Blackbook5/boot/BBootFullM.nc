...
implementation {
  ...
  command bool BBoot.isBooted() {
    return (call BlackbookState.getState() != S_BOOT_BUSY) && (call BlackbookState.getState() != S_BOOT_RECOVERING_BUSY);
  }
  ...
  event void Checkpoint.recovered(flashnode *focusedNode, result_t result) {
    if(call BlackbookState.getState() == S_BOOT_RECOVERING_BUSY) {
      ...
      if(currentNode->state == NODE_DELETED) {
        currentNode->state = NODE_EMPTY;
      }      
  ...
  task void parseCurrentNode() {
    ...
    if(currentNodeMeta.magicNumber == META_EMPTY) {
      ...
      currentNode->state = NODE_BOOTING;
      if(currentFile != NULL) {
  ...
  task void recoverNodes() {
    if(currentIndex < call NodeMap.getMaxNodes()) {
      ...
      if(currentNode->state == NODE_BOOTING) {
        if(call Checkpoint.recover(currentNode)) {
          return;
        }
      }
