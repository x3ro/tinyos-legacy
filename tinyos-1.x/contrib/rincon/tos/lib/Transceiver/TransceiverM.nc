...
module TransceiverM {
  ...
  uses {
    ...
    interface BareSendMsg as SendRadio;
    interface ReceiveMsg as ReceiveUart;
    interface ReceiveMsg as ReceiveRadio;
  }
}
...
implementation {
  ...
  /***************** Radio Events ****************/
  event result_t SendRadio.sendDone(TOS_MsgPtr m, result_t result) {
    if(m == &msg[nextSendMsg].tosMsg) {
      signal Transceiver.radioSendDone[m->type](m, result);
      sendDone();
    }
    return SUCCESS;
  }
  
  event TOS_MsgPtr ReceiveRadio.receive(TOS_MsgPtr m) {
    if(call PacketFilter.filterPacket(m, RADIO)) {
      return signal Transceiver.receiveRadio[m->type](m);
    }
    return m;
  }
  
  /***************** UART Events ****************/
  event result_t SendUart.sendDone(TOS_MsgPtr m, result_t result) {
    if(m == &msg[nextSendMsg].tosMsg) {
      signal Transceiver.uartSendDone[m->type](m, result);
      sendDone();
    }
    return SUCCESS;
  }
  ...
  event TOS_MsgPtr ReceiveUart.receive(TOS_MsgPtr m) {
    ...
    if(call PacketFilter.filterPacket(m, UART)) {
      return signal Transceiver.receiveUart[m->type](m);
    }
    return m;
  }
   ...
   */
  task void sendMsg() {
    if(msg[nextSendMsg].sendMethod == RADIO) {
      if(!call SendRadio.send(&msg[nextSendMsg].tosMsg)) {
        post sendMsg();
      }
    } else {
      if(!call SendUart.send(&msg[nextSendMsg].tosMsg)) {
        post sendMsg();
      }
    }
  }
