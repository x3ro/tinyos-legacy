...
implementation {
  ...
  command result_t FlashBridge.erase[uint8_t id](uint16_t sector) {
    if(sector > call FlashSettings.getTotalEraseUnits() - 1) {
      return FAIL;
    }
    ...
    if(call State.requestState(S_ERASE)) {
      currentClient = id;
      currentAddr = sector;
      return call HALSTM25P.sectorErase(currentAddr * call FlashSettings.getEraseUnitSize());
  ...
  command uint32_t FlashSettings.getFlashSize() {
    return STM25P_ERASE_UNIT_LENGTH * STM25P_ERASE_UNITS;
  }
  ...
  command uint32_t FlashSettings.getEraseUnitSize() {
    return STM25P_ERASE_UNIT_LENGTH;
  }
  ...
  command uint32_t FlashSettings.getWriteUnitSize() {
    return STM25P_WRITE_UNIT_LENGTH;
  }
  ...
  command uint8_t FlashSettings.getEraseUnitSizeLog2() {
    return STM25P_ERASE_UNIT_LENGTH_LOG2;
  }
  ...
  command uint8_t FlashSettings.getWriteUnitSizeLog2() {
    return STM25P_WRITE_UNIT_LENGTH_LOG2;
  }
  ...
  void write() {
    if(currentTotal < currentLen) {
      currentAmount = call FlashSettings.getWriteUnitSize() - ((currentAddr + currentTotal) % call FlashSettings.getWriteUnitSize());
