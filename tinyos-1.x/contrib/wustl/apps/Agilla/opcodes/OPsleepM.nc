// $Id: OPsleepM.nc,v 1.8 2006/05/23 21:05:59 chien-liang Exp $
...
implementation {  
  ...
  command result_t StdControl.init() {
    ...
    call QueueI.init(&waitQueue, OP_SLEEP_M);       
    //call QueueI.init(&waitQueue);       
    //dbg(DBG_USR1, "OPsleepM queue is at address %i\n", (int)&waitQueue);
    return SUCCESS;
  }
    ...
    if (call OpStackI.popOperand(context, &arg)) {      
      ...
      if (i != -1) {        
        ...
        case 0:
          ...
          #endif       
          call Timer0.start(TIMER_ONE_SHOT, amount);
          break;
        case 1:
          #if DEBUG_OP_SLEEP
            dbg(DBG_USR1, "OPSlEEP(%i): Starting sleep timer 1 for %i ms\n", context->id.id, amount);
          #endif       
          call Timer1.start(TIMER_ONE_SHOT, amount);
          break;
        //case 2:
        default:
          #if DEBUG_OP_SLEEP
          ...
          #endif       
          call Timer2.start(TIMER_ONE_SHOT, amount);
          break;
        /*default:
          #if DEBUG_OP_SLEEP
  ...
  inline result_t handleTimer(int i) {    
    #if DEBUG_OP_SLEEP
  ...
  event result_t Timer0.fired() {      
    return handleTimer(0);
  }
  
  event result_t Timer1.fired() {      
    return handleTimer(1);
  }
  
  event result_t Timer2.fired() {      
    return handleTimer(2);
  }
