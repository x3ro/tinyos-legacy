// $Id: OPcisnbrM.nc,v 1.3 2006/05/18 19:58:41 chien-liang Exp $
  ...
  command result_t BytecodeI.execute(uint8_t instr, AgillaAgentContext* context) {                  
    ...
    if (call OpStackI.popOperand(context, &arg)) {      
      if (arg.vtype & AGILLA_VAR_V)   
        context->condition = call NeighborListI.isNeighbor(arg.value.value);   
      else if (arg.vtype & AGILLA_VAR_L)
        context->condition = call NeighborListI.isNeighbor(call LocationMgrI.getAddress(&arg.loc));   
      else {
