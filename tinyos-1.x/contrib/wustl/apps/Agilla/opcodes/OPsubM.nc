// $Id: OPsubM.nc,v 1.2 2006/05/18 19:58:41 chien-liang Exp $
...
implementation {
  result_t subLocLoc(AgillaAgentContext* context, AgillaLocation* loc1, 
  ...
  result_t subLocVal(AgillaAgentContext* context, AgillaValue* value, 
    ...
    if (call Stacks.popOperand(context, &arg1))
    {
      if (call Stacks.popOperand(context, &arg2)) 
      {
        ...
        else if (arg1.vtype == AGILLA_TYPE_VALUE && arg2.vtype == AGILLA_TYPE_LOCATION) 
        {
          ...
          if (subLocVal(context, &arg1.value, &arg2.loc, &newLoc)) 
          {
        ...
        {
          AgillaLocation newLoc;
          if (subLocVal(context, &arg2.value, &arg1.loc, &newLoc)) 
          {
            call Stacks.pushLocation(context, &newLoc);
            return SUCCESS;
          } else
        ...
        {
          AgillaLocation newLoc;                
          if (subLocLoc(context, &arg1.loc, &arg2.loc, &newLoc)) 
          {
            call Stacks.pushLocation(context, &newLoc);
            return SUCCESS;
          } else
