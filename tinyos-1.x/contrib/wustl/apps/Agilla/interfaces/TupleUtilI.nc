// $Id: TupleUtilI.nc,v 1.2 2006/05/18 19:58:41 chien-liang Exp $
...
interface TupleUtilI {
   ...
   */
  command result_t getField(AgillaTuple* tuple, uint16_t pos, AgillaVariable* var);  
   ...
   */
  command result_t getTuple(AgillaAgentContext* context, AgillaTuple* tuple);
  
  /**
   * Determines whether a template field matches a tuple's field. 
   */  
  command result_t fMatches(AgillaVariable* tmplt, AgillaVariable* tuple);
  
  /**
   * Determines whether the two fields are identical.
   */
  command result_t fEquals(AgillaVariable* field1, AgillaVariable* field2);
   ...
   * owner is a match.
   */
  command result_t tMatches(AgillaTuple* template, AgillaTuple* tuple, bool isRemove);
  
  /**
   * Returns SUCCESS if the two tuples are equal, FALSE otherwise.
   */
  command result_t isEqual(AgillaTuple* tuple1, AgillaTuple* tuple2);
   ...
   */
  command result_t pushTuple(AgillaTuple* tuple, AgillaAgentContext* context);
  
  /**
   * Returns SUCCESS if all of the fields in the tuple are one of the types specified.
   */
  command result_t checkFieldTypes(AgillaAgentContext* context, 
   ...
   */
  command result_t printField(AgillaVariable* field);
   ...
   */  
  command result_t printTuple(AgillaTuple* tuple);
   ...
   */
  command result_t createHostIDTuple(AgillaTuple* tuple);  
   ...
   * @param tuple A pointer to the tuple to modify.
   */  
  command result_t createAgentIDTuple(AgillaTuple* tuple, AgillaAgentID id);
}
