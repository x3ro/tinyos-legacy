// $Id: GridMgrI.nc,v 1.2 2006/05/18 19:58:41 chien-liang Exp $
...
interface GridMgrI {
   ...
   * multiple hops away.
   */
  command result_t isPossibleNbr(AgillaLocation loc);
   ...
   */
  command result_t addr2Loc(uint16_t addr, AgillaLocation* loc);
   ...
   */
  command result_t loc2addr(AgillaLocation* loc, uint16_t* addr);
   ...
   */
  command result_t getPosNbr(uint16_t i, AgillaLocation* loc);

  /**
   * Returns the number of possible neighbors.
   */
  command result_t numPosNbrs(uint16_t* numPosNbr);
}
