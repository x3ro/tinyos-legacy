// $Id: AgentMgrM.nc,v 1.30 2006/07/02 18:45:48 chien-liang Exp $
...
implementation {
  ...
  inline uint16_t getNewID() {
    uint16_t newid;
    newid = (uint16_t)TOS_LOCAL_ADDRESS;
    newid = newid << 8;
    newid += idCount++;
    return newid;
  }
    ...
    uint16_t final_dest, uint8_t op)
  {
    AgillaAgentID id = context->id;
      ...
      call ExpLoggerI.sendTraceQid(context->id.id, TOS_LOCAL_ADDRESS, AGENT_MIGRATING, TOS_LOCAL_ADDRESS, SUCCESS, loc);
    #endif
    ...
    uint16_t dest)
  {
    if (dest == TOS_LOCAL_ADDRESS) {
      ...
      // send the agent migration trace to the base station
        ...
        call ExpLoggerI.sendTraceQid(context->id.id, TOS_LOCAL_ADDRESS, AGENT_MOVED, TOS_LOCAL_ADDRESS, SUCCESS, loc);
      #endif
