// $Id: OpStackM.nc,v 1.4 2006/05/18 19:58:40 chien-liang Exp $
...
implementation {
  ...
  command result_t OpStackI.pushAgentID(AgillaAgentContext* context, AgillaAgentID* aID){
    AgillaVariable var;
  ...
  command result_t OpStackI.pushString(AgillaAgentContext* context, uint16_t string) {
    AgillaVariable var;
  ...
  command result_t OpStackI.pushLocation(AgillaAgentContext* context, AgillaLocation* loc) {
    AgillaVariable var;
  ...
  command result_t OpStackI.pushReading(AgillaAgentContext* context, uint16_t type, int16_t reading) {
    AgillaVariable var;
  ...
  command result_t OpStackI.pushType(AgillaAgentContext* context, uint16_t type) {
    AgillaVariable var;
  ...
  command result_t OpStackI.pushReadingType(AgillaAgentContext* context, uint16_t rtype) {
    AgillaVariable var;
  ...
  command result_t OpStackI.pushValue(AgillaAgentContext* context,  int16_t val) {
    AgillaVariable var;
  ...
  command result_t OpStackI.pushOperand(AgillaAgentContext* context, AgillaVariable* var) {
    ...
    if (varSize > 0) {
      if (context->opStack.sp + varSize + 1 < AGILLA_OPSTACK_SIZE) {
        nmemcpy((void*)&context->opStack.byte[context->opStack.sp], (void*)&var->loc, varSize);          
  ...
  command result_t OpStackI.popOperand(AgillaAgentContext* context, AgillaVariable* var) {
    if (context->opStack.sp == 0) {
      ...
      if (varSize > 0) {
        context->opStack.sp -= varSize;      
        nmemcpy((void*)&var->loc, (void*)&context->opStack.byte[context->opStack.sp], varSize);
  ...
  command result_t OpStackI.peekOperand(AgillaAgentContext* context, AgillaVariable* var) {
    if (context->opStack.sp == 0) {
      dbg(DBG_ERROR, "ERROR: OpStackI.popOperand: Tried to pop off end of stack.\n");
      call ErrorMgrI.error(context, AGILLA_ERROR_STACK_UNDERFLOW);
      var->vtype = AGILLA_TYPE_INVALID;
      ...
      varSize = call VarUtilI.getSize(context, var->vtype);      
      if (varSize > 0) {
        nmemcpy((void*)&var->loc, (void*)&context->opStack.byte[context->opStack.sp-varSize-1], varSize);                     
        return SUCCESS;
      }
    }
    return FAIL;
    ...
    AgillaOpStackMsg* osMsg) 
  {
    ...
    if (remaining > AGILLA_OS_MSG_SIZE)
      ...
      numBytes = remaining;
    nmemcpy((void*)&osMsg->data[0], (void*)&context->opStack.byte[startAddr], numBytes);
   ...
   */
  command result_t OpStackI.saveMsg(AgillaAgentContext* context, AgillaOpStackMsg* osMsg) {
    if (context->opStack.byte[osMsg->startAddr] != AGILLA_TYPE_INVALID) {
      ...
      nmemcpy((void*)&context->opStack.byte[startAddr], (void*)&osMsg->data[0], numBytes);
      return SUCCESS;
    }
  }
   ...
   */
  command result_t OpStackI.reset(AgillaAgentContext* context) {
    int i;
  ...
  command result_t OpStackI.toString(AgillaAgentContext* context) {
    #ifdef __CYGWIN__
    ...
    while(sp > 0) {
      ...
      if (varSize > 0) {
        sp -= varSize;
        nmemcpy((void*)&var.loc, (void*)&context->opStack.byte[sp], varSize);    
