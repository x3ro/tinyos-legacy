// $Id: ReceiveStateM.nc,v 1.9 2006/05/18 19:58:40 chien-liang Exp $
...
module ReceiveStateM {
  uses {
    ...
    interface AgentMgrI;
    interface ReceiveMsg as Rcv_State;
    interface SendMsg as Send_State_Ack;
...
implementation { 
  ...
  {
    TOS_MsgPtr ackMsg = call MessageBufferI.getMsg();
    if (ackMsg != NULL) 
    {
      struct AgillaAckStateMsg *aMsg = (struct AgillaAckStateMsg *)ackMsg->data;      
      aMsg->id = *id;
      aMsg->accept = acpt;
      if (!call Send_State_Ack.send(addr, sizeof(AgillaAckStateMsg), ackMsg))
   ...
   */
  event TOS_MsgPtr Rcv_State.receive(TOS_MsgPtr m) 
  {
    AgillaStateMsg* sMsg = (AgillaStateMsg*)m->data;
    #if DEBUG_AGENT_RECEIVER
      dbg(DBG_USR1, "ReceiveStateM: Rcv_State.receive: Received STATE for agent id = %i.\n", sMsg->id.id);
    #endif     
    
    if (call AgentMgrI.isPresent(&sMsg->id)) {
      #if DEBUG_AGENT_RECEIVER
    ...
    if (call CoordinatorI.isArriving(&sMsg->id)) {
      #if DEBUG_AGENT_RECEIVER
     ...
     */
    if (call CoordinatorI.allocateBuffer(&sMsg->id, sMsg->codeSize)) 
    {
  ...
  event result_t Send_State_Ack.sendDone(TOS_MsgPtr m, result_t success) 
  { 
