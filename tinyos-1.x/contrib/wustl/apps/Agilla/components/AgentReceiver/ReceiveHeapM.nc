// $Id: ReceiveHeapM.nc,v 1.3 2006/05/18 19:58:40 chien-liang Exp $
...
module ReceiveHeapM {
  uses {
    ...
    interface HeapMgrI;
    interface ReceiveMsg as Rcv_Heap;
...
implementation {
  ...
  inline void sendHeapAck(uint16_t addr, AgillaAgentID *id, uint8_t acpt, uint8_t addr1) 
  {
    ...
    if (msg != NULL) 
    {
      ...
      aMsg->accept = acpt;
      aMsg->addr1 = addr1;
      if (!call Send_Heap_Ack.send(addr, sizeof(AgillaAckHeapMsg), msg))
        call MessageBufferI.freeMsg(msg);
      if (acpt == AGILLA_REJECT) call CoordinatorI.resetBuffer(id);
    }
  } 
  
  event TOS_MsgPtr Rcv_Heap.receive(TOS_MsgPtr m) 
  {
    AgillaHeapMsg* hpMsg = (AgillaHeapMsg*)m->data;    
    AgillaAgentInfo* inBuf;
    
    if (!call CoordinatorI.isArriving(&hpMsg->id)) {       
    ...
    if (!call CoordinatorI.stopTimer(&hpMsg->id)) {
      #if DEBUG_AGENT_RECEIVER
    ...
    if (call HeapMgrI.saveMsg(inBuf->context, hpMsg)) {
      inBuf->numHpMsgs--;               
  ...
  event result_t Send_Heap_Ack.sendDone(TOS_MsgPtr m, result_t success) 
  { 
