// $Id: NeighborListI.nc,v 1.7 2006/05/18 19:58:40 chien-liang Exp $
...
interface NeighborListI {
   ...
   */
  command result_t isNeighbor(uint16_t nbr);
   ...
   * FALSE otherwise.
   */
  command result_t getNeighbor(uint16_t i, uint16_t* nbr);
  ...
  command result_t getRandomNeighbor(uint16_t *nbr);
     ...
     */
    command result_t getClosestNeighbor(uint16_t *nbr);
  #endif
