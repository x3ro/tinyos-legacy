// $Id: RxnMgrM.nc,v 1.9 2006/05/18 19:58:40 chien-liang Exp $
...
implementation {
   ...
   */
  command result_t RxnMgrI.runRxnMgr() {
    int i;
   ...
   */
  command result_t RxnMgrI.registerRxn(Reaction* rxn) {
    int i;
    ...
    for (i = 0; i < REACTION_MGR_BUFFER_SIZE; i++) {
      if (!buffer[i].isUsed) {
        ...
        if (call TS.rdp(&tuple)) {
          ...
          #if DEBUG_RXNMGR
            dbg(DBG_USR1, "RxnMgrI.registerRxn(...): Immediately found a tuple that matched the reaction!\n");
            call TupleUtilI.printTuple(&tuple);
          #endif        
          
          signal RxnMgrI.rxnFired(&buffer[i].rxn, &tuple);
        }*/
  ...
  command result_t RxnMgrI.deregisterRxn(Reaction* rxn) {
  ...
  command result_t RxnMgrI.getRxn(AgillaAgentID* id, uint16_t which, Reaction* rxn) {
    uint16_t i, count = 0;
  ...
  command result_t RxnMgrI.isRegistered(Reaction* rxn) {    
   ...
   */
  command result_t RxnMgrI.flush(AgillaAgentID* id) {
    int16_t i;
    for (i = 0; i < REACTION_MGR_BUFFER_SIZE; i++) {
      if (buffer[i].isUsed && id->id == buffer[i].rxn.id.id)                    
        buffer[i].isUsed = FALSE;           
   ...
   */
  event result_t TS.byteShift(uint16_t from, uint16_t amount) {
    int16_t i;
    for (i = 0; i < REACTION_MGR_BUFFER_SIZE; i++) {
  ...
  event result_t TS.newTuple(AgillaTuple* tuple) {
    if (call AgentExecutorI.isIdle())
      call RxnMgrI.runRxnMgr();
    return SUCCESS;
  }
  
  //default event result_t RxnMgrI.rxnFired(Reaction* rxn, AgillaTuple* tuple) {
  //  return SUCCESS;
  //}
}
