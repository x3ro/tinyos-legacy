// $Id: TupleUtilM.nc,v 1.8 2006/05/18 19:58:40 chien-liang Exp $
...
implementation {
  ...
  command int16_t TupleUtilI.addField(AgillaTuple* tuple, int16_t pos, AgillaVariable* var) {
    ...
    if (varSize != -1) {
      ...
      if (pos + varSize <= AGILLA_MAX_TUPLE_SIZE) {
        nmemcpy((void*)&tuple->data[pos], (void*)&var->loc, varSize);          
        return pos + varSize;
      } else {
   ...
   */
  command result_t TupleUtilI.getField(AgillaTuple* tuple, uint16_t fieldNum, AgillaVariable* var) {
    ...
    varSize = call VarUtilI.getSize(NULL, var->vtype);
    if (varSize != -1) {
      nmemcpy((void*)&var->loc, (void*)&tuple->data[pos], varSize);         
      return SUCCESS;
    } else
      return FAIL;    
  }  
   ...
   */
  command result_t TupleUtilI.fMatches(AgillaVariable* tempField, AgillaVariable* tupField) {
   ...
   */
  command result_t TupleUtilI.fEquals(AgillaVariable* field1, AgillaVariable* field2) {
    if (field1->vtype == field2->vtype) { // actual match
      switch(field1->vtype) {
      case AGILLA_TYPE_AGENTID:   
        return field1->id.id == field2->id.id;
      case AGILLA_TYPE_LOCATION:  
   ...
   * owner is a match.
   */
  command result_t TupleUtilI.tMatches(AgillaTuple* template, AgillaTuple* tuple, bool isRemove) {
    ...
    for (i = 0; i < template->size; i++) {
      ...
      if (call TupleUtilI.getField(template, i, &tempField) && 
          call TupleUtilI.getField(tuple, i, &tupField)) 
      {
        if (!call TupleUtilI.fMatches(&tempField, &tupField)) {
          #if DEBUG_TUPLE_UTIL
   ...
   */  
  command result_t TupleUtilI.isEqual(AgillaTuple* tuple1, AgillaTuple* tuple2) {
    ...
    for (i = 0; i < tuple1->size; i++) {
      if (call TupleUtilI.getField(tuple1, i, &tup1Field) &&
        call TupleUtilI.getField(tuple2, i,  &tup2Field))
      {
        if (!call TupleUtilI.fEquals(&tup1Field, &tup2Field)) return FAIL;
      } else
        return FAIL;
    }
    return SUCCESS;
   ...
   */  
  command result_t TupleUtilI.getTuple(AgillaAgentContext* context, AgillaTuple* tuple) {
    uint16_t i, size;
    int pos = 0;
    ...
    if(call OpStackI.popOperand(context, &field)) {   
   ...
   */
  command result_t TupleUtilI.checkFieldTypes(AgillaAgentContext* context, 
    AgillaTuple* tuple, uint16_t type) 
  {
    ...
    for (i = 0; i < tuple->size; i++) {
      if (call TupleUtilI.getField(tuple, i, &field)) {      
        if (!(field.vtype & type)) {
   ...
   */
  command result_t TupleUtilI.pushTuple(AgillaTuple* tuple, AgillaAgentContext* context) {
    int i;
    AgillaVariable field;
   ...
   */  
  command result_t TupleUtilI.printField(AgillaVariable* field) {
    switch(field->vtype) {
    case AGILLA_TYPE_VALUE:
   ...
   */    
  command result_t TupleUtilI.printTuple(AgillaTuple* tuple) 
  {
    int i;
    AgillaVariable field;
   ...
   */
  command result_t TupleUtilI.createHostIDTuple(AgillaTuple* tuple) {
    ...
    field2.value.value = TOS_LOCAL_ADDRESS;   
   ...
   * @param tuple A pointer to the tuple to modify.
   */  
  command result_t TupleUtilI.createAgentIDTuple(AgillaTuple* tuple, AgillaAgentID id) {
    AgillaVariable field1, field2;
    int i = 0;    
  ...
  //event result_t SendMsg.sendDone(TOS_MsgPtr M, result_t success) { return SUCCESS;}  // debug
}
