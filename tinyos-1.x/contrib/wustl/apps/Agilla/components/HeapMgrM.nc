// $Id: HeapMgrM.nc,v 1.5 2006/05/18 19:58:40 chien-liang Exp $
...
implementation {
    ...
    while (addr < AGILLA_HEAP_SIZE  && !msgFull)
    {
      ...
      if (vType != AGILLA_TYPE_INVALID) 
      {
        ...
        if (varSize != 0 && i + varSize + 2 < AGILLA_HEAP_MSG_SIZE) 
        {
          ...
          nmemcpy((void*)&heapMsg->data[i], (void*)&context->heap.pos[addr].loc, varSize);
          i += varSize;
        } else 
          msgFull = TRUE;
      }
   ...
   * a duplicate.
   */
  command result_t HeapMgrI.saveMsg(AgillaAgentContext* context, AgillaHeapMsg* heapMsg) 
  {
    uint8_t addr, vtype;
    uint16_t i = 0;
    bool done;
    ...
    while (!done) 
    {
      ...
      if (varSize != 0) 
      {
        ...
        nmemcpy((void*)&context->heap.pos[addr].loc, (void*)&heapMsg->data[i], varSize);
        i += varSize;
   ...
   */
  command result_t HeapMgrI.reset(AgillaAgentContext* context) {
    int i;
   ...
   */
  command result_t HeapMgrI.hasHeap(AgillaAgentContext* context) {
