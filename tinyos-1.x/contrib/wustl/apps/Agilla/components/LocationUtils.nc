// $Id: LocationUtils.nc,v 1.3 2006/05/18 19:58:40 chien-liang Exp $
...
implementation {
   ...
   */
  command result_t LocationUtilI.isBcast(AgillaLocation *l) {
    return l->x == BCAST_X && l->y == BCAST_Y;
  }

  /**
   * Returns SUCCESS if the specified location is the UART address.
   */
  command result_t LocationUtilI.isUart(AgillaLocation *l) {
    return l->x == UART_X && l->y == UART_Y;
  }
  
  /**
   * Returns SUCCESS if the two locations are equal, FALSE otherwise.
   */
  command result_t LocationUtilI.equals(AgillaLocation *l1, AgillaLocation *l2) {
