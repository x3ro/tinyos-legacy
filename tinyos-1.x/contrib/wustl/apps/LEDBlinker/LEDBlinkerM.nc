...
implementation {
  ...
  void setLEDs(uint16_t whichLEDs) {
    if (whichLEDs & 1) call Leds.redToggle();
    if (whichLEDs & 2) call Leds.greenToggle();
    if (whichLEDs & 4) call Leds.yellowToggle();  
  }
   ...
   */
  command result_t LEDBlinkerI.blink(uint16_t whichLEDs, uint16_t count, uint16_t period) 
  {
    if (!blinking) 
    {
      blinking = TRUE;
      ...
      setLEDs(_whichLEDs);
      _curr = 1;      
      call Timer.start(TIMER_ONE_SHOT, _period);
      return SUCCESS;
    } else
      return FAIL;
  }
   ...
   */
  default event result_t LEDBlinkerI.blinkDone() {  
    return SUCCESS;
  }
}
