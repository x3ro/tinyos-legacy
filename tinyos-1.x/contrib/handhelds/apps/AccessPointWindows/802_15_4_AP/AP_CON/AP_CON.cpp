...
// AP_CON talks to TAP-Win32 driver and to a COM port where a device (SHIMMER or Telos)
...
void do_ti_info(struct TelosInform * pti)
	...
	// that part we print all the time ...
...
*/
	...
	case TELOS_DATA:
		...
		DWORD to_write = count - 1 + sizeof(ETH_HEADER);
		BOOL b_adapt = WriteFile(g_h_adapt, &data_adapt, to_write, (DWORD*) &n_adapt, &o_adapt);
		// maybe multitask a little :)
		if(g_verbose & VERBOSE_DATA)
			{
			printf("\nAP received DATA message length %d\n", count - 1 );
			}
		DWORD m = 0;
		...
		if( (g_verbose & VERBOSE_DATA) && (m != to_write))
			{
			printf("\nonly injected on the network %d instead of %d\n", m, to_write);
			}
		}
...
static int telos_ap_unesc(struct telos_ap *telos, unsigned char s, unsigned char * p_dest)
	...
	case ESCAPE_BYTE:
		...
		if(g_verbose & VERBOSE_UNESC_ALL)
			{
			printf(" %02x*", s);
			}
		return -1;
	...
	try
		...
		while(!g_flag_exit)
			...
			if(done_comm)
				...
				BOOL b = ReadFile(g_h_comm, (BYTE*) &data, 1, (DWORD*) &n, &o_comm);
				if(b)
					{
					//??
			...
			do
				...
				if(rw == WAIT_OBJECT_0 + 1)
					...
					// as a way to recover from errors we do that now in all cases
					done_comm = true;
					//printf("read from comm\n");
					//printf("%02x ", data);
					if(n == 1 || m == 1)
						{
						if( g_verbose & VERBOSE_COMM_ALL )
							{
							printf("[%02x]", data);
							}
						...
						if( (i < limit) && (g_verbose & VERBOSE_UNESCAPE) )
							{
							printf("telos_ap_unesc returned %d\n", i);
							}
						}
					else
						{
						printf("\nError reading from COM n=%d m=%d\n", n, m);
						}
					}
