...
class LowLevel:
    ...
    def SetDTR(self, level, invert):
        ...
        if DEBUG > 2: print 'DTR %d      level=%d invert=%d' % (level ^ invert, level, invert)
        if invert:
    ...
    def SetRTS(self, level, invert):
        ...
        if DEBUG > 2: print '     RTS %d level=%d invert=%d' % (level ^ invert, level, invert)
        if invert:
    ...
    def DS4510SetSCL(self, level):
        self.serialport.setRTS(not level)

    def DS4510SetSDA(self, level):
        self.serialport.setDTR(not level)

    def DS4510I2CStart(self):
	self.DS4510SetSDA(1)
        time.sleep(1e-6)
	self.DS4510SetSCL(1)
        time.sleep(1e-6)
    ...
    def DS4510I2CWriteBit(self, bit):
	self.DS4510SetSCL(0)
	self.DS4510SetSDA(bit)
        time.sleep(2e-6)
	self.DS4510SetSCL(1)
        time.sleep(1e-6)
	self.DS4510SetSCL(0)

    def DS4510I2CWriteByte(self, byte):
        self.DS4510I2CWriteBit( byte & 0x80 );
        self.DS4510I2CWriteBit( byte & 0x40 );
        self.DS4510I2CWriteBit( byte & 0x20 );
        self.DS4510I2CWriteBit( byte & 0x10 );
        self.DS4510I2CWriteBit( byte & 0x08 );
        self.DS4510I2CWriteBit( byte & 0x04 );
        self.DS4510I2CWriteBit( byte & 0x02 );
        self.DS4510I2CWriteBit( byte & 0x01 );
        self.DS4510I2CWriteBit( 0 );  # "acknowledge"

    def DS4510I2CWriteAddress(self):
	# currently assuming A0 = 0
	A0=0
	addressByte = 0xa0 | (A0<<1)
	self.DS4510I2CWriteByte(addressByte)

    def DS4510I2CWriteCmd(self, addr, cmdbyte):
	self.DS4510I2CStart()
        self.DS4510I2CWriteAddress()
        self.DS4510I2CWriteByte(addr)
	self.DS4510I2CWriteByte(cmdbyte)
	self.DS4510I2CStop()

    def zloxReset(self,invokeBSL=0):

	# currently assuming DS4510 chip has A0 = 0 
	PULLUP_REG = 0xF0
	RST_DELAY_REG = 0xF1
	IO3_REG = 0xF4
	IO2_REG = 0xF5
	...
	IO0_REG = 0xF7
	CONFIG_REG = 0xF9
	CONFIG_SRAM_WRITES = 0x10
	CONFIG_EEPROM_WRITES = 0x00
	PULLUP0_ACTIVE = 0x01
	PULLUP1_ACTIVE = 0x02
	PULLUP2_ACTIVE = 0x04
	PULLUP3_ACTIVE = 0x08
	PULLUPS_ACTIVE = PULLUP0_ACTIVE|PULLUP1_ACTIVE|PULLUP2_ACTIVE|PULLUP3_ACTIVE
	RST_REG = IO0_REG
	TCK_REG = IO1_REG

        if DEBUG > 1: sys.stderr.write("* zloxReset(invokeBSL=%s)\n" % invokeBSL)

	# make writes to chip non permanent
	self.DS4510I2CWriteCmd(CONFIG_REG,CONFIG_SRAM_WRITES) 
	# turn on internal pullups
	self.DS4510I2CWriteCmd(PULLUP_REG,PULLUPS_ACTIVE) 
	self.DS4510I2CWriteCmd(RST_REG,1) 

	if invokeBSL:
	    self.DS4510I2CWriteCmd(TCK_REG,1) 
	    self.DS4510I2CWriteCmd(RST_REG,0) 
	    self.DS4510I2CWriteCmd(TCK_REG,0) 
	    self.DS4510I2CWriteCmd(TCK_REG,1)
	    self.DS4510I2CWriteCmd(TCK_REG,0) 
 	    self.DS4510I2CWriteCmd(RST_REG,1) 
	    self.DS4510I2CWriteCmd(TCK_REG,1) 
	...
	# make writes to chip  permanent for rst state
	self.DS4510I2CWriteCmd(CONFIG_REG,CONFIG_SRAM_WRITES) 	
	time.sleep(0.02)
	self.DS4510I2CWriteCmd(PULLUP_REG,PULLUPS_ACTIVE) 
	time.sleep(0.02)
...
General options:
  ...
  --shimmer2            Just --invert-test --invert-reset, for ftdi um232r
  --span                Just --invert-test --invert-reset, for ftdi um2232d
