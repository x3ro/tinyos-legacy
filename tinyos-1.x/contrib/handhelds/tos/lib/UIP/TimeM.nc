...
#define NUM_YEARS 1
  ...
  command result_t StdControl.start() {
    g_tick_local_time = call LocalTime.read();
    call Timer.start(TIMER_REPEAT, 10*1024L);
    signal Time.tick();
    return SUCCESS;
  }
  ...
  command void Time.setCurrentTime(time_t current_time){
    ...
    g_local_time = call LocalTime.read();
  }
  ...
  void dotick(int force) {
    time_t tick = call LocalTime.read();
  ...
  */
  ...
  command result_t Time.gmtime(const time_t *timer, struct tm *ptm)
  {
  ...
  command result_t Time.localtime(const time_t *timer, struct tm *result)
  {
  ...
  command result_t Time.asctime(const struct tm *tm, char *buf, int buflen)
  {
    char *out = buf;
  ...
  command result_t Time.time(time_t *timer)
  {
    // 1/32768 seconds since last NTP response
    uint64_t o = call LocalTime.read() - g_local_time;
