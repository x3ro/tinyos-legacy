/*
 * file:        PageNANDM.nc
 * description: address expanded for 128MB NAND Flash usage, few bugs fixed
 * author:      Gong Zhang, Beihang Univ. Computer Science Dept. 5/2011 
 */
 ...
 * $Id: PageNANDM.nc,v 1.1 2011/08/14 16:37:54 dukenunee Exp $
 *
...
#define MAX_ADDR 5
     ...
     * any results.
     */
    result_t SendRcv(uint8_t cmd, int naddr, uint8_t *addr,int nout, uint8_t *out, int nin, uint8_t *in) 
	{
		atomic 
		{
			int i;
       ...
       // we_lo = we_hi & ~(1 << NAND_WE_BIT);
        ...
        MAKE_PWBUS_INPUT();//Read from the NAND
...
#ifdef FAL_DEBUG
        {
	        call Console.printf0("Data read:");
	        for (i = 0; i <= count; i++)
            {
        		call Console.printf0(temp[i]);
    ...
    command result_t PageNAND.write(
										nandpage_t page, 
										nandoffset_t offset, 
										void *data, 
										nandoffset_t len
	...
	{
        uint8_t xaddr[MAX_ADDR];
		uint8_t alen = data_addr(page, offset, xaddr);
        uint8_t cmd;
         ...
         */
		call Console.printf1("add %x2", xaddr[0]);
		call Console.printf1("%x2", xaddr[1]);
		call Console.printf1("%x2", xaddr[2]);
		call Console.printf1("%x2", xaddr[3]);
		call Console.printf1("%x2 ", xaddr[4]);
        if (
				SendRcv(NAND_DATA_IN, alen, xaddr, len, data, NO_DATA) &&
    ...
    command result_t PageNAND.read(	nandpage_t page, 
									nandoffset_t offset, 
									void *data, 
									nandoffset_t len) 
	{
        uint8_t xaddr[MAX_ADDR], 
		alen = data_addr(page, offset, xaddr);
        uint8_t cmd;
        post ReadRespond();		
	    if (small_page) {
	        if (offset > 0x1ff)
	...
	return SendRcv(NAND_ID_READ, 1, &addr, NO_DATA, 16, id);
    }
