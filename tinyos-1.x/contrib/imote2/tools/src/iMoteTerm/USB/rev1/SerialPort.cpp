...
CSerialPort::CSerialPort() : m_rxThread(NULL), m_txThread(NULL), m_sendQueue()
{
...
bool CSerialPort::OpenPort(const char *lpcstrPort, DCB *pDCB)
{
	if(m_hComm != INVALID_HANDLE_VALUE)
	{
		//port is apparently already openned, abort this operation
		TRACE("openPort called with non INVALID_HANDLE_VALUE value for COM port handle\n");
		return false;
	}
	...
	TRACE("Successfully set Comstate\n");
	...
	TRACE("Successfully set CommMask\n");
	...
	{
		TRACE("Error Setting CommTimeouts.  GetLastError returned %d",GetLastError());
		//PrivateClosePort();
		ClosePort();
		return false;
	}
	TRACE("Successfully set CommTimeouts\n");
	TRACE("Succesfully initialized port %s\n",lpcstrPort);
	...
	TRACE("ModemStatus value = %d\n",modemstatus);
	TRACE("CTS_ON = %d, DSR_ON = %d, RING_ON = %d, RLSD_ON = %d\n",MS_CTS_ON, MS_DSR_ON, MS_RING_ON, MS_RLSD_ON);
...
bool CSerialPort::WriteData(BYTE *data, DWORD datalen){
	...
	TRACE("Attempting to send command \"%s\"\n",tempData);
...
UINT CSerialPort::CommThreadRxFunc(LPVOID pParam)
{
	...
	*************************************************************************/
	...
	while(pPort->IsConnected())
	{
		...
		buffer = new unsigned char[requestedBytesRead];
...
{
	if(IsConnected()){
		TRACE("Requesting COM port closure\n");
		m_bConnected = false;
		if(m_rxThread != NULL){
...
bool CSerialPort::SetBreak(bool On)
{
	...
	if(bret)
	{
		TRACE("Succeded: Set or clear break\r\n");
	}
	else
	{
		TRACE("Failed: Set or clear break\r\n");
	}
	return bret;
}
