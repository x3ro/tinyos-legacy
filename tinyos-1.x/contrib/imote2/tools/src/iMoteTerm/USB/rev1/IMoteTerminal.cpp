							   ...
							   , m_addrString(_T(""))
{
	...
	m_lastSendFile = NULL;
	m_sendFileStatus = new CIMoteStatus(this);
	m_sendFileBusy = false;
	...
	m_addrRichEditControl.SetWindowText("0x00200000");

	binaryFile = INVALID_HANDLE_VALUE;
...
void CIMoteTerminal::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_RICHEDIT_TERM_ADDR, m_addrRichEditControl);
	DDX_Control(pDX, IDC_BUTTON_TERM_EXECUTE, m_executeButton);
	DDX_Control(pDX, IDC_STATIC_TERM_ADDR, m_addrStatic);
	DDX_Text(pDX, IDC_RICHEDIT_TERM_ADDR, m_addrString);
}
...
BEGIN_MESSAGE_MAP(CIMoteTerminal, CDialog)
	...
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BUTTON_TERM_EXECUTE, OnBnClickedButtonTermExecute)
	ON_COMMAND(ID_TERM_EDIT_EXECUTE, OnTermEditExecute)
END_MESSAGE_MAP()
...
BOOL CIMoteTerminal::OnInitDialog()
{
	...
	m_outputRichEditControl.Create(ES_MULTILINE | ES_READONLY | WS_CHILD |
		...
		CRect(100, (m_usb != NULL?66:41), 480 - 10, 280 - 10), this, IDC_RICHEDIT_OUTPUT);
	...
	if(m_usb != NULL){
		m_executeButton.ShowWindow(SW_SHOW);
		m_executeButton.EnableWindow(1);
		m_addrStatic.ShowWindow(SW_SHOW);
		m_addrRichEditControl.ShowWindow(SW_SHOW);
		...
		m_executeButton.ShowWindow(SW_HIDE);
		m_executeButton.EnableWindow(0);
		m_sendBreakButton.ShowWindow(SW_SHOW);
		m_sendBreakButton.EnableWindow(1);
		m_addrRichEditControl.ShowWindow(SW_HIDE);
		m_addrStatic.ShowWindow(SW_HIDE);
	}
...
{
	if(m_usb != NULL){ //usb device
		if(m_attached)
			m_detachedStatic.ShowWindow(SW_HIDE);
		else{
			m_detachedStatic.ShowWindow(SW_SHOW);
			m_sendFileButton.EnableWindow();
			m_sendFileStatus->ShowWindow(SW_HIDE);
			m_sendFileBusy = false;
			free(m_lastSendFile);
			m_lastSendFile = NULL;
		}
...
void CIMoteTerminal::Disconnect(){
	if(m_usb == NULL){ //serial device
...
{	
	...
	if(m_sizeCount == 0)
		...
		if(m_usb != NULL)
			m_addrRichEditControl.SetWindowPos(NULL , 100, 0, cx - 234, 23, SWP_NOREPOSITION | SWP_NOMOVE | SWP_SHOWWINDOW);
		m_outputRichEditControl.SetWindowPos(NULL, 100, (m_usb != NULL?66:41), cx - 110, cy - (m_usb != NULL?66:41) - 10, SWP_NOREPOSITION | SWP_NOMOVE | SWP_SHOWWINDOW);
		WidthAdjustment();
	}
	...
	if(isFlagged(type, _BIT(IMOTE_HID_TYPE_H))){
		...
		switch((USBin->type >> IMOTE_HID_TYPE_L) & 3){
			case IMOTE_HID_TYPE_L_BYTE:
				...
				if(USBin->i != *(InputReport + IMOTE_HID_NI))
				{
...
void CIMoteTerminal::checkBinary(BYTE *data, DWORD BytesToCheck, DWORD Packet){
	//DWORD bytesWritten = 0;
	if(m_lastSendFileValid){
		*(data + BytesToCheck - 1) = 1;
		*(m_lastSendFileIndex + m_lastSendFile + BytesToCheck - 1) = 1;
		if(memcmp(m_lastSendFile + m_lastSendFileIndex,data,BytesToCheck) != 0){
			m_lastSendFileValid = false;
			//MessageBox("Burn Invalid");
			TRACE("Burn is Invalid\r\n");
		}
	}
	m_lastSendFileIndex+=BytesToCheck;

	/*if(Packet == 0){
		CString fName;
		GetWindowText(fName);
		fName.Append(".bin.out");
		binaryFile = CreateFile(fName, GENERIC_WRITE,0,NULL,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,NULL);
	}
	...
	if(binaryFile == INVALID_HANDLE_VALUE){
		...
		WriteFile(binaryFile, data, BytesToWrite, &bytesWritten, NULL);*/
}
...
void CIMoteTerminal::OnBnClickedButtonTermSendFile(){
	...
	UpdateData();
	if(m_sendFileBusy){
		MessageBox("File In Progress");
		return;
	}
	if(m_usb != NULL && !m_bTextFile){
		if(0 == sscanf(m_addrString,"%x", &addr)){
			MessageBox("Enter a valid hexadecimal address please");
			return;
		}
		if(addr < 0x00200000){
			MessageBox("Enter a valid address");
			return;
		}
	}
	...
	if(((m_usb != NULL && m_usb->IsConnected()) || (m_usb == NULL && m_serialport->IsConnected())) &&
		m_fileOpenName != ""){
			...
			if(m_usb != NULL && !m_bTextFile){
				DWORD temp;
				BYTE msg[5] = {0,(BYTE)(addr>>24),(BYTE)((addr<<8)>>24),(BYTE)((addr<<16)>>24),(BYTE)(addr & 0xff)};
				SendData(msg, 5, (IMOTE_HID_TYPE_MSC_BLOADER << IMOTE_HID_TYPE_MSC) | IMOTE_HID_TYPE_CL_BINARY);
				if(length <= IMOTE_HID_TYPE_L_BYTE_SIZE)
					temp = length / IMOTE_HID_BYTE_MAXPACKETDATA;
				else if(length <= IMOTE_HID_TYPE_L_SHORT_SIZE)
					temp = length / IMOTE_HID_SHORT_MAXPACKETDATA;
				else if(length <= IMOTE_HID_TYPE_L_INT_SIZE)
					temp = length / IMOTE_HID_INT_MAXPACKETDATA;
				m_sendFileStatus->UpdateStatus(0,temp,0,temp,0xF);
				m_sendFileStatus->ShowWindow(SW_SHOW);
				m_sendFileButton.EnableWindow(false);
				m_sendFileBusy = true;
				m_executeButton.EnableWindow(0);
				m_lastSendFile = buffer;
				m_lastSendFileValid = true;
				m_lastSendFileIndex = 0;
			}
...
void CIMoteTerminal::OnBnClickedButtonTermExecute(){
	OnTermEditExecute();
}

void CIMoteTerminal::OnTermEditExecute(){
	UpdateData();
	if(m_usb == NULL)
		return;
	if(!m_usb->IsConnected()){
	...
	if(m_sendFileBusy){
		MessageBox("Currently burning an image");
		return;
	}
	DWORD addr;
	if(0 == sscanf(m_addrString,"%x", &addr)){
	...
	BYTE msg[5] = {2,(BYTE)(addr>>24),(BYTE)((addr<<8)>>24),(BYTE)((addr<<16)>>24),(BYTE)(addr & 0xff)}; //addr
	SendData(msg, 5, (IMOTE_HID_TYPE_MSC_BLOADER << IMOTE_HID_TYPE_MSC) | IMOTE_HID_TYPE_CL_BINARY);
}

int CIMoteTerminal::processJTPacket(USBdata *USBin, BYTE validBytes)
{

	if((USBin->type & 0x3) == IMOTE_HID_TYPE_CL_GENERAL){ //signal for packet here
		if(USBin->len > 0){
			CFile rawfile, datafile;

			sampleHeader_t *pSH = (sampleHeader_t *)USBin->data;
			TRACE("Received a sampleRecord for channel %d w/outpUOM = %d containing %d samples: Progress = %d/%d\r\n", pSH->channelId, pSH->outputUOM, pSH->numSamples, pSH->sampleOffset, pSH->totalSamples);
			CString tempStr;
			tempStr.Format("Mote_%#x_Channel_%d_Collection_%d.", pSH->logicalNodeId, pSH->channelId, pSH->sequenceID);

			if(pSH->sampleOffset == 0){
				datafile.Open(tempStr + "dat",CFile::modeCreate| CFile::modeWrite | CFile::shareDenyNone);
				rawfile.Open(tempStr + "raw",CFile::modeCreate| CFile::modeWrite | CFile::shareDenyNone);
			}
			else{
				datafile.Open(tempStr + "dat",CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite | CFile::shareDenyNone);
				rawfile.Open(tempStr + "raw",CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite | CFile::shareDenyNone);

			}
			datafile.SeekToEnd();
			rawfile.SeekToEnd();
			
			signed short *data = (signed short *)(USBin->data + sizeof(sampleHeader_t));
			for(int i=0; i<pSH->numSamples; i++){
				float temp = data[i]*pSH->ADCScale + pSH->ADCOffset;
				datafile.Write(&temp, sizeof(float));
			}
			datafile.Close();
			rawfile.Write(USBin->data,USBin->len);
			rawfile.Close();
		}
		...
		BYTE keyType = IMOTE_HID_TYPE_CL_BINARY | (IMOTE_HID_TYPE_MSC_BLOADER << IMOTE_HID_TYPE_MSC);
		//TRACE("received binary ish thing type: %x, %d %d\r\n",USBin->type, USBin->i, USBin->n);
		if((USBin->type & keyType) == keyType){
			if(validBytes == 5){
				if(*((DWORD *)USBin->data) != 0){
					Disconnect();
					switch(*((DWORD *)USBin->data)){
						case 1:
							BufferAppend("Flash Erase Operation Failed; Disconnecting\r\n");
							break;
						case 2:
							BufferAppend("Flash Write Operation Failed; Disconnecting\r\n");
							break;
						case 3:
							BufferAppend("Flash Write Size Operation Failed; Disconnection\r\n");
							break;
					}
				}
			}
			else
				m_sendFileStatus->UpdateStatus(*((DWORD *)USBin->data),0,0,0,1);
		}
		else{
			m_sendFileStatus->UpdateStatus(0,0,USBin->i,0,4);
			checkBinary(USBin->data, USBin->len, USBin->i);
			if(USBin->i == USBin->n){
				m_sendFileStatus->ShowWindow(SW_HIDE);
				BYTE msg[1] = {1};
				SendData(msg, 1, (IMOTE_HID_TYPE_MSC_BLOADER << IMOTE_HID_TYPE_MSC) | IMOTE_HID_TYPE_CL_BINARY);
				if(m_lastSendFileValid)
					m_lastSendFileValid = false;
				else
					MessageBox("InvalidBytes burn");

				free(m_lastSendFile);
				m_lastSendFile = NULL;
				m_sendFileButton.EnableWindow();
				m_executeButton.EnableWindow();
				m_sendFileBusy = false;
			}
		}
		...
		USBin->data = (BYTE *)realloc(USBin->data, USBin->len+ 1);
		assert(USBin->data != NULL);
		USBin->data[USBin->len] = '\0';
		USBin->len++;
