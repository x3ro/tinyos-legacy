// $Id: DirectedGraphPlugin.java,v 1.1 2004/08/18 00:12:42 lakiolen Exp $
...
//    String drawingHood;
    ...
    public void handleEvent(SimEvent event) {
		...
		if(!knownHoods.contains(hood)){
		    ...
		    //System.out.println("added new selected");
		    //System.out.println("known hoods: " + knownHoods);
		    //System.out.println("length: " + length);
                    graphList.setListData(knownHoods);
		    //System.out.println("set list");
                    graphList.setSelectedIndices(newSelected);
		    //System.out.println("updated graph list");
...
//        parameterPane.add(cb);
	...
	graphList.addListSelectionListener(new DirectedGraphPlugin.jlListener());
        ...
        while (enum.hasMoreElements()) {
            ...
            state.getSelectedMoteSimObjects().contains(state.getMoteSimObject(neighborhood.nodeID))) {
                ...
                while (neighbors.hasMoreElements()) {
                    ...
                    CoordinateAttribute neighborCoord = neighbor.getCoordinate();
                    graphics.setColor(neighborID.color);
                    int x1=(int) cT.simXToGUIX(coord.getX());
                    int x2=(int) cT.simXToGUIX(neighborCoord.getX());
                    int y1=(int) cT.simYToGUIY(coord.getY());
                    int y2=(int) cT.simYToGUIY(neighborCoord.getY());
		    int dx = x2 - x1;
		    int dy = y2 - y1;
		    if (dx == 0) {
			x1 += neighborID.offset;
			x2 += neighborID.offset;
		    }
		    else if (dy == 0) {
			y1 += neighborID.offset;
			y2 += neighborID.offset;
		    }
		    else {
    ...
    class jlListener implements ListSelectionListener {
