function stopAllMsgs(varargin)
% A generic function to disconnect all matlab message handlers.
% If no arguments are given, we disconnect all matlab message handlers of
% applications in the global structure APPS.
% Otherwise, we only disconnect matlab message handlers of the structure
...
%
% usage: stopAllMsgs()
%       stopAllMsgs(MAGLIGHT) % MAGLIGHT is a structure containing 
...
for i = 1:length(msgNames)
    for k = 1:length(appStruct.Comm.(msgNames{i}).Handler)
        stopReceiving(appStruct.Comm.(msgNames{i}).Handler{k},appStruct.Comm.(msgNames{i}).Msg);
    end
end

function usage
disp('usage: stopAllMsgs()');
disp('    stopAllMsgs(MAGLIGHT) % MAGLIGHT is a structure containing');
