function rTurnRaw(turn,speed,time,moteID)
% rTurnRaw(turn,speed,time,moteID)
%
% Allows for command line driving of COTSBOT.  Drives with wheels turned
...
%         speed                 Raw speed number setting for COTSBOT
...
driveRaw_dat.timer{j}(1) = timer('TimerFcn',{'driveRaw_helper1', moteID, ...
  	    RC.REVERSE, turn, speed, time}, 'StartDelay', minDelay);
...
if (time ~= 0)
  driveRaw_dat.timer{j}(4) = timer('TimerFcn',{'driveRaw_helper4', moteID, ...
	    RC.REVERSE, turn, speed, time}, 'StartDelay', time);
