function revRaw(speed,time,moteID)
% revRaw(speed,time,moteID)
%
% Allows for command line driving of COTSBOT.  Drives straight in the
% reverse direction at speed setting for a specified amount of time.
...
% Usage: revRaw(50,2,1)         speed = 50, time = 2 sec, moteid = 1
%        revRaw(50,0)           speed = 50, time = inf, moteid = broadcast
...
driveRaw_dat.timer{j}(1) = timer('TimerFcn',{'driveRaw_helper1', moteID, ...
  	    RC.REVERSE, RC.STRAIGHT, speed, time}, 'StartDelay', minDelay);
...
if (time ~= 0)
  driveRaw_dat.timer{j}(4) = timer('TimerFcn',{'driveRaw_helper4', moteID, ...
	    RC.REVERSE, RC.STRAIGHT, speed, time}, 'StartDelay', time);
