...
#     previous function.
#
# Modified to work with TinyOS in these ways:
# - looks for the ChangeLog in ../../doc/ChangeLog
# - looks for differences in .nc files as well as .c files
#
use diagnostics;
use strict;
...
#
# For TinyOS, we need to move to the the doc directory; we use 
# TOSDIR to do this. We only have one possible arg: -tosdir
# -LKW
#
my $TOSDIR;
my $i;
...
# Remove trailing / from TOSDIR, if any 
...
my $file;
open DIFF, "cd ..; cvs -fq diff -N |" or die "The cvs diff failed: $OS_ERROR.\n";
while (<DIFF>)
  {
    $file = $1 if /^Index: (\S+)/;
    if (defined $file
...
# LKW: took out because we don't review
