// $Id: XnpM.nc,v 1.1.1.1 2007/11/05 19:10:06 jpolastre Exp $
  ...
  event TOS_MsgPtr ReceiveMsg.receive(TOS_MsgPtr msgptr) {
	...
	switch (cCommand)	     //was [4]
	  ...
	  case CMD_GET_CIDMISSING:
	    ...
	    // reply to PC side if finished scan of EEPROM 
	    // and the query is destined to this mote  
  ...
  *****************************************************************************/
  ...
  command result_t Xnp.NPX_SET_IDS(){
    ...
    if((wTemp=eeprom_read_word ((uint16_t *)AVREEPROM_LOCALID_ADDR)) != 0xFFFF )
      atomic TOS_LOCAL_ADDRESS = wTemp;
#ifdef JTAG
    atomic TOS_LOCAL_ADDRESS = TOS_LOCAL;               //Jtag debug
#endif
    return SUCCESS;
  }
