...
#define MCU_ENABLE_IRQ(x) call MSP430Interrupt.clear(); call MSP430Interrupt.edge(FALSE); call MSP430Interrupt.enable();
#define MCU_DISABLE_IRQ(x) call MSP430Interrupt.disable(); call MSP430Interrupt.clear();
...
**************************************************************************************************/
    ...
    uint8_t receivedPacket[128], receivedBuffer[128];
    uint8_t * receivedPacketPtr, * receivedBufferPtr;

    /**********************************************************************
     * Init
     *********************************************************************/
    command result_t HALCC2420Control.init() 
    {
        call HPLCC2420Control.init();
    ...
    task void initTask()
    {
        ...
        ******************************************/
        ...
        if (!foundId)
        {
            ...
            ieeeAddress[4] = 0;
            ieeeAddress[5] = 0;
            ieeeAddress[6] = TOS_LOCAL_ADDRESS >> 8;
            ieeeAddress[7] = TOS_LOCAL_ADDRESS;
        }
...
debug_fsm();
     ...
     * Start
     *********************************************************************/
    command result_t HALCC2420Control.start()
    {       
     ...
     * Stop
     *********************************************************************/
    command result_t HALCC2420Control.stop()
    {
...
**************************************************************************************************/
    ...
    command result_t HALCC2420.sendPacket(uint8_t * packet) 
    {
    ...
    command result_t HALCC2420.setChannel(uint8_t channel) 
    {
    ...
    command result_t HALCC2420.setTransmitPower(uint8_t power)
    {
        if (power > 100) 
            return FAIL;
        else {
     ...
     * rxEnable
     *********************************************************************/
    command result_t HALCC2420.rxEnable()
    {
     ...
     * rxDisable
     *********************************************************************/
    command result_t HALCC2420.rxDisable()
    {
...
**************************************************************************************************/
     ...
     * setShortAddress
     *********************************************************************/
    command result_t HALCC2420.setAddress(mac_addr_t *addr)
    {
        shortAddress = *addr;
     ...
     * setPANAddress
     *********************************************************************/
    command result_t HALCC2420.setPanAddress(mac_addr_t *addr)
    {
        panid = *addr;
     ...
     *********************************************************************/
    command result_t HALCC2420.addressFilterEnable()
    {
    ...
    command result_t HALCC2420.addressFilterDisable()
    {
...
**************************************************************************************************/
    ...
    task void receivedPacketSignalTask();
    ...
    async event void MSP430Interrupt.fired()
    {
        ...
        if (transmitInProgress) {
            ...
            if (flushBufferTaskPosted) {
                ...
                call StdOut.print("Unhandled interrupt\r\n");
            }
        }
    ...
    task void receivedPacketTask()
    {
        ...
        ***********************************************/
        ...
        if (correlation & FCS_CRC_OK_MASK || TRUE) 
        {
            ptr = receivedBufferPtr;
            receivedBufferPtr = receivedPacketPtr;
            receivedPacketPtr = ptr;
            
            post receivedPacketSignalTask();
        }
    ...
    task void receivedPacketSignalTask()
    {
        receivedBufferPtr = signal HALCC2420.receivedPacket(receivedBufferPtr);
    }
