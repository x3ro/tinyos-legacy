...
// note:this function blocks &waits until EEPROM is available
    ...
    pAddr = &wProgramID;	 //the prog id			   
	eeprom_write_byte (AVREEPROM_PID_ADDR, *pAddr++);//lsbyte
	eeprom_write_byte (AVREEPROM_PID_ADDR+1, *pAddr);//msbyte
...
void fNPXStartDownload(TOS_MsgPtr msgptr)  {
  ...
  TOS_MsgPtr *wptr;
		...
		wptr = &(msgptr->data[TS_PID]);
		wProgramID = (uint16_t)*wptr++;
		wNofCapsules= (uint16_t)*wptr;
