...
// This is a test program for BMAC!
...
module fabAppM 
{
  uses {
    ...
    interface SysTime as MsecClock;
...
//***  Global variables/constants
  ...
  enum {
    CLUSTER_HEAD_ID = 10, // DEBUGGING - just for bmac test!
  ...
  uint32_t init_ts_delay_msec = (uint32_t) 600000; 
  ...
  task void sendStatsMsg()
  {
	...
	sizeof(fabAppMsg) + sizeof(Statistics_t), &data)) {
#ifdef MEASURE_STATS
  ...
  {
    fabAppMsg* fm = (fabAppMsg *) m->data;

    if (m->length < sizeof(fabAppMsg)) {
      dbg(DBG_USR1, "fab ERROR: msglen: %d <= %d\n", m->length, 
	sizeof(fabAppMsg)); 
      return m;
    }
  ...
  event result_t WakeUpTimer.fired()
  {
    ...
    send_pending = FALSE; // fix because sometimes radio seems to get wedged
