 ...
 *
 * $Id: NMEA.h,v 1.1 2004/12/10 05:46:12 husq Exp $
 *
 */
...
#endif
#endif
bool extract_num_sats_m(char *data) 	{ return (10*(data[0]-'0') + (data[1]-'0'));}
bool extract_hours_m(char *data) 		{ return (10*(data[0]-'0') + (data[1]-'0'));}
bool extract_minutes_m(char *data) 		{ return (10*(data[2]-'0') + (data[3]-'0'));}
bool extract_dec_sec_m(char *data) 		{ return (10*(data[4]-'0') +  (data[5]-'0') + 0.1*(data[7]-'0') \
                                      		+ 0.01*(data[8]-'0')                                  \
				      						+ 0.001*(data[9]-'0'));}
bool extract_Lat_deg_m(char *data) 		{ return (10*(data[0]-'0') + (data[1]-'0'));}
bool extract_Lat_dec_min_m(char *data) 	{ return (10*(data[2]-'0') +  (data[3]-'0') + 0.1*(data[5]-'0') \
                                      		+ 0.01*(data[6]-'0') + 0.001*(data[7]-'0') + 0.0001*(data[8]-'0'));}
bool extract_Long_deg_m(char *data) 	{ return (100*(data[0]-'0') + 10*(data[1]-'0') + (data[2]-'0'));}
bool extract_Long_dec_min_m(char *data) { return (10*(data[3]-'0') +  (data[4]-'0') + 0.1*(data[6]-'0') \
				      						+ 0.01*(data[7]-'0') + 0.001*(data[8]-'0') + 0.0001*(data[9]-'0'));}
...
bool is_gga_string_m(char *ns){return (ns[3]=='G')&&(ns[4]=='G')&&(ns[5]=='A');};
...
typedef struct gga_data {
  ...
  // Magic and mote_id could go into the NMEA_Data struct.
  //uint8_t magic;
  //uint16_t mote_id;
