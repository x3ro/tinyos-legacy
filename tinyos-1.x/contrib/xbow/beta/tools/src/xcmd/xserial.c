 ...
 * 
 * $Id: xserial.c,v 1.1 2004/10/07 19:33:13 mturon Exp $
 */
...
#ifdef __arm__
#define SERIAL_DEVICE     "/dev/tts/2"   // default port to use
#else
...
{
    int err = 0;
    err = write(serline, buffer, length);
    if (err < 0) {
...
int xserial_port_write_packet(int serline, char *buffer, int cnt) 
{
    // Use single byte reads for now as they are more stable.
