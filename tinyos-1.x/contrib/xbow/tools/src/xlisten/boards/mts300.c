 ...
 * 
 * $Id: mts300.c,v 1.25 2005/01/27 06:37:51 husq Exp $
 */
#include "../xdb.h"
#include "../xsensors.h"

// packetid =1 ; sersor data
typedef struct {
    uint16_t vref;
    uint16_t thermistor;
    uint16_t light;
...
extern XPacketHandler mts300_packet_handler;
...
const char *mts300_db_create_rule = 
...
void mts300_log_raw(XbowSensorboardPacket *packet) 
{
    ...
    if (!mts300_packet_handler.flags.table_init) {
	int exists = xdb_table_exists(table);
	if (!exists) {
	    // Create results table.
	    sprintf(command, mts300_db_create_table, table, "");
	    xdb_execute(command);
	    // Create last result cache
	    sprintf(command, mts300_db_create_table, table, "_L");
	    xdb_execute(command);
	    
	    // Add rule to populate last result table
	    sprintf(command, mts300_db_create_rule, table, table, table, table);
	    xdb_execute(command);

	    // Add results table to query log.
	    int q_id = XTYPE_MTS300, sample_time = 99000;
	    sprintf(command, "INSERT INTO task_query_log "
		    "(query_id, tinydb_qid, query_text, query_type, "
		    "table_name) VALUES (%i, %i, 'SELECT nodeid,parent,voltage,temp,light,mic,"
	...
	mts300_packet_handler.flags.table_init = 1;
    }
    ...
    sprintf(command, 
	    ...
	    //timestring,
	    packet->node_id, packet->parent, 
	    data->vref, data->thermistor, data->light, data->mic
	);
...
void mts310_log_raw(XbowSensorboardPacket *packet) 
{
	...
	if (!exists) {
	    ...
	    sprintf(command, "INSERT INTO task_query_log "
		    ...
		    "voltage,temp,light,accel_x,accel_y,mag_x,mag_y,mic,"
		    "SAMPLE PERIOD %i', 'sensor', '%s')", q_id, q_id,
		    sample_time, table);
	    xdb_execute(command);
    ...
    sprintf(command, 
	    "INSERT into %s "
	    "(result_time,nodeid,parent,voltage,temp,light,"
	    "accel_x,accel_y,mag_x,mag_y,mic)"
	    " values (now(),%u,%u,%u,%u,%u,%u,%u,%u,%u,%u)", 
	    table,
	    //timestring,
	    packet->node_id, packet->parent, 
	    data->vref, data->thermistor, data->light,
	    data->accelX, data->accelY, data->magX, data->magY, data->mic
	);
