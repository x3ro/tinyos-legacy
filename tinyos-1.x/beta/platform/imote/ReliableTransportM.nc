...
module ReliableTransportM {
   provides {
      ...
      interface VarRecv[uint8_t app];
      interface VarSend[uint8_t app];
   }
   uses {
...
//Task States
  ...
  task void processPostSend(){
   ...
   command result_t VarSend.postSend[uint8_t app](void *handle, uint16_t numBytes, 
						  uint16_t destAddr){
...
#ifdef RELIABLE_TRANSPORT_DEBUG
       sprintf(temp, "ReliableTransport: Calling VarSend.postSend numbytes = %d app ID %d\n", numBytes, app);
       DisplayStr(temp);
#endif
   ...
   command result_t 
   VarSend.pullSegDone[uint8_t app](void *handle, uint16_t msgOffset, result_t result){
...
#ifdef RELIABLE_TRANSPORT_DEBUG
       sprintf(temp, "ReliableTransport: GOT a pullsegDone, app id %d\n", appId);
       DisplayStr(temp);
#endif
  ...
  task void abortSend(){
   ...
   command result_t VarSend.abortSend [uint8_t app](void *handle){
     if(appId != app) {
        return FAIL;
     }
...
//	   rNack->rHead.appId = APP_ID_RELIABLE_TRANSPORT;
	   rNack->rHead.appId = relConnParams[i].appId;
   ...
   task void processNackTimers(){
	   ...
	   if(relConnParams[i].unusedIndex == FALSE){
	       if(relConnParams[i].nackMainTimer  <= (2*REL_MAIN_TIMER)){
		   ...
		   if(relConnParams[i].nackShortTimer  <= (2*REL_MAIN_TIMER)){
		       SendNack(i);
		   }
	       }
		   ...
		   if(relConnParams[i].nackDataTimeout  <= (2*REL_MAIN_TIMER)){
		       signal VarRecv.recvDone[relConnParams[i].appId](relConnParams[i].handle, FAIL);
		       relConnParams[i].unusedIndex = TRUE;
		       return;
		   }
	       }
		   ...
		   if((relConnParams[i].lastAckResendTimer%REL_RESEND_ACK_RESEND_TIMER) 
		      == 0){
		       if(relConnParams[i].lastAckResendTimer  <= (2*REL_MAIN_TIMER)){
   ...
   task void checkTimer(){
	   ...
	   if((connReqResendTimer%REL_RESEND_CONN_REQ_TIMER) == 0){
		   ...
		   //		   signal SendLargeBuf.sendLargeBufDone[currApp](sendBuf,FAIL);
		   sendPacketPending  = FALSE;
		   signal VarSend.sendDone[appId](connHandle,FAIL);
                   connInProgress = FALSE;
		   currConnAcc = FALSE;
	       }
	       else{
   ...
   task void processSendDone(){
	 ...
	 return;
	   ...
	   rData->rHead.appId = appId;
	   rData->rHead.tId   = tId;
	   rData->fragIndex   = currFrag;  

	   if(!signal VarSend.pullSegReq[appId](connHandle, currFrag*maxFragSize, 
   ...
   event result_t GenericPacket.SendDone(uint8_t* Buffer, result_t success) {
       uint16_t len;
       RelDataPtr localData;
       localData = (RelDataPtr)call 
                 GenericPacket.GetPayloadStart((uint8_t*)Buffer, &len);
	   ...
	   if(localData->rHead.mtc == REL_DATA){
#if IMOTE
             call StatsLogger.BumpCounter(NUM_RT_SEND_DATA, 1);
#else
	     call ReportPacketEvent.PacketEvent(RT_SEND_DATA,0); // can't get seq number, and not useful at this level anyway
#endif
	   }
	   ...
	   if(Buffer == msg){
	       call GenericPacket.FreeBuffer(Buffer);
	       sendPending = FALSE;
	       if(localData->rHead.mtc == REL_NACK){
#if IMOTE
                 call StatsLogger.BumpCounter(NUM_RT_SEND_NACK, 1);
#else
		 call ReportPacketEvent.PacketEvent(RT_SEND_NACK,0); // can't get seq number, and not useful at this level anyway
#endif
	       }
	       return SUCCESS;
	   }
  ...
  //for debugging
   ...
   task void processLastAck(){
	 ...
	 return;
	   ...
	   signal VarSend.sendDone[appId](connHandle, SUCCESS);
           connInProgress = FALSE;
	   currConnAcc = FALSE;
       }
   ...
   task void processConnAcc(){
	...
	//call Leds.greenOn();   	
       ...
       if(!signal VarSend.pullSegReq[appId](connHandle, 0, sendFragStore, maxFragSize)){
	   sendState = REL_SEND_PACKETS_DONE;
	   call GenericPacket.FreeBuffer(sendMsg);
       }
  ...
  task void saveState(){
   ...
   result_t ReceiveCmdReceive(wsnAddr Source, void* payload, uint16_t len) {
       ...
       case REL_NACK:
#if IMOTE
         call StatsLogger.BumpCounter(NUM_RT_RECV_NACK, 1);
#else
	 call  ReportPacketEvent.PacketEvent(RT_RECV_NACK,rNack->seq); // can't get seq number, and not useful at this level anyway
#endif
...
//	   rConnRej->rHead.appId = APP_ID_RELIABLE_TRANSPORT;
	   rConnRej->rHead.appId = relConnParams[connAccIndex].appId;
...
//	       rLastAck->rHead.appId = APP_ID_RELIABLE_TRANSPORT;
	       rLastAck->rHead.appId = relConnParams[i].appId;
...
//	   rConnAcc->rHead.appId                  = APP_ID_RELIABLE_TRANSPORT;
	   rConnAcc->rHead.appId                  = relConnParams[connAccIndex].appId;
...
task void processRelData(){
    ...
    if(signal VarRecv.putSegReq[relConnParams[lastDataIndex].appId](relConnParams[lastDataIndex].handle, 
...
result_t ReceiveReceive(wsnAddr source, void *payload, uint16_t len){
	...
	if(i != REL_MAX_CONN_NUM || ((rConn->fragSize %2) != 0)){
		...
		relConnParams[i].ackBitmap[j] = 0;
            ...
            DisplayStr("ReliableTransport: conn req\r\n");
	    if(!signal VarRecv.recvReq[relConnParams[lastConnIndex].appId](relConnParams[lastConnIndex].addr,
    ...
    case REL_DATA:
#if IMOTE
      call StatsLogger.BumpCounter(NUM_RT_RECV_DATA, 1);
#else
      call  ReportPacketEvent.PacketEvent(RT_RECV_DATA, 0); // can't get seq number, and not useful at this level anyway
#endif
	dbg(DBG_USR1, 
	...
	if(i != REL_MAX_CONN_NUM){
	    ...
	    relConnParams[i].unusedIndex = TRUE;
	    signal VarRecv.recvDone[relConnParams[i].appId](relConnParams[i].handle, SUCCESS);
	}
   ...
   command result_t VarRecv.acceptRecv[uint8_t app](void *Handle, uint8_t TransactionID){
       if(relConnParams[TransactionID].appId != app) {
           return FAIL;
       }
       if(sendConnAccTask == REL_TASK_DONE){
   ...
   command result_t VarRecv.rejectRecv[uint8_t app](uint8_t TransactionID){
       if(relConnParams[TransactionID].appId != app) {
           return FAIL;
       }
       if(sendConnRejTask == REL_TASK_DONE){
   ...
   command result_t VarRecv.putSegDone[uint8_t app](void *Handle, uint16_t MsgOffset){
       return SUCCESS;
   }
   command result_t VarRecv.abortRecv[uint8_t app](void *Handle){
       return SUCCESS;
   }
   ...
   default event result_t VarSend.pullSegReq[uint8_t app](void *Handle, uint16_t                       MsgOffset, uint8_t *SegBuf, uint8_t SegSize){
       return SUCCESS;
   }
