...
/*************** STATE TRANSITION CODE ****************************/
     ...
     async command void PowerState.ADCon() {
	  if(!prof_on) return;
	  dbg(DBG_USR1, "POWER: Mote %d ADC ON at %lld \n", NODE_NUM,
	      tos_state.tos_time);
     }

     async command void PowerState.ADCoff() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d ADC OFF at %lld \n", NODE_NUM,
	      tos_state.tos_time);
     }

     async command void PowerState.ADCdataReady() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d ADC DATA_READY at %lld \n", NODE_NUM,
	      tos_state.tos_time);
     }
     ...
     async command void PowerState.ADCsample(uint8_t port) {
	  ...
	  case TOS_ADC_CC_RSSI_PORT:
	       if(!prof_on) return; 
	       dbg(DBG_USR1, "POWER: Mote %d ADC SAMPLE RSSI_PORT at %lld \n", NODE_NUM,
		   tos_state.tos_time);
	  case TOS_ADC_PHOTO_PORT:
	       if(!prof_on) return; 
	       dbg(DBG_USR1, "POWER: Mote %d ADC SAMPLE PHOTO_PORT at %lld \n", NODE_NUM,
		   tos_state.tos_time);
	       break;
	  case TOS_ADC_TEMP_PORT:
	       if(!prof_on) return; 
	       dbg(DBG_USR1, "POWER: Mote %d ADC SAMPLE TEMP_PORT at %lld \n", NODE_NUM,
		   tos_state.tos_time);
	       break;
	  case TOS_ADC_MIC_PORT:
	       if(!prof_on) return; 
	       dbg(DBG_USR1, "POWER: Mote %d ADC SAMPLE MIC_PORT at %lld \n", NODE_NUM,
		   tos_state.tos_time);
	       break;
	  case TOS_ADC_ACCEL_X_PORT:
	       if(!prof_on) return; 
	       dbg(DBG_USR1, "POWER: Mote %d ADC SAMPLE ACCEL_X_PORT at %lld \n", NODE_NUM,
		   tos_state.tos_time);
	       break;
	  case TOS_ADC_ACCEL_Y_PORT:
	       if(!prof_on) return; 
	       dbg(DBG_USR1, "POWER: Mote %d ADC SAMPLE ACCEL_Y_PORT at %lld \n", NODE_NUM,
		   tos_state.tos_time);
	       break;
	  case TOS_ADC_MAG_X_PORT:
	       if(!prof_on) return; 
	       dbg(DBG_USR1, "POWER: Mote %d ADC SAMPLE MAG_X_PORT at %lld \n", NODE_NUM,
		   tos_state.tos_time);
	       break;
	  case TOS_ADC_MAG_Y_PORT:
	       if(!prof_on) return; 
	       dbg(DBG_USR1, "POWER: Mote %d ADC SAMPLE MAG_Y_PORT at %lld \n", NODE_NUM,
		   tos_state.tos_time);
	       break;
	  default:
	       if(!prof_on) return; 
	       dbg(DBG_USR1, "POWER: Mote %d ADC SAMPLE unknown_port_%i at %lld \n", NODE_NUM,
     ...
     async command void PowerState.redOn() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d LED_STATE RED_ON at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
  
     async command void PowerState.redOff() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d LED_STATE RED_OFF at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
  
     async command void PowerState.greenOn() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d LED_STATE GREEN_ON at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
  
     async command void PowerState.greenOff() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d LED_STATE GREEN_OFF at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
  
     async command void PowerState.yellowOn() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d LED_STATE YELLOW_ON at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
  
     async command void PowerState.yellowOff() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d LED_STATE YELLOW_OFF at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
     ...
     async command void PowerState.radioTxMode() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d RADIO_STATE TX at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }

     async command void PowerState.radioRxMode() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d RADIO_STATE RX at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }

     async command void PowerState.radioRFPower(uint8_t power_level) {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d RADIO_STATE SetRFPower %X at %lld\n", NODE_NUM, power_level,
	      tos_state.tos_time);
     }

     async command void PowerState.radioStart() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d RADIO_STATE ON at %lld\n", NODE_NUM, 
	      tos_state.tos_time);
     }

     async command void PowerState.radioStop() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d RADIO_STATE OFF at %lld\n", NODE_NUM, 
	      tos_state.tos_time);
     }
     ...
     async command void PowerState.cpuState(uint8_t sm) {
	  ...
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d CPU_STATE %s at %lld\n", NODE_NUM, cpu_power_state[sm],
	      tos_state.tos_time);
     ...
     async command void PowerState.sensorPhotoOn() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d SENSOR_STATE PHOTO ON at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }

     async command void PowerState.sensorPhotoOff() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d SENSOR_STATE PHOTO OFF at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }

     async command void PowerState.sensorTempOn() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d SENSOR_STATE TEMP ON at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
                                                                                         
     async command void PowerState.sensorTempOff() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d SENSOR_STATE TEMP OFF at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }

     async command void PowerState.sensorAccelOn() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d SENSOR_STATE ACCEL ON at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
                                                                                         
     async command void PowerState.sensorAccelOff() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d SENSOR_STATE ACCEL OFF at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
     ...
     async command void PowerState.eepromReadStart() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d EEPROM READ START at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
  
     async command void PowerState.eepromReadStop() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d EEPROM READ STOP at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }

     async command void PowerState.eepromWriteStart() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d EEPROM WRITE START at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
  
     async command void PowerState.eepromWriteStop() {
	  if(!prof_on) return; 
	  dbg(DBG_USR1, "POWER: Mote %d EEPROM WRITE STOP at %lld\n", NODE_NUM,
	      tos_state.tos_time);
     }
...
//       if(!prof_on) return; 
// dbg(DBG_USR1, "POWER: Mote %d SNOOZE START at %lld\n", NODE_NUM,
//           tos_state.tos_time);
     }

     async command void PowerState.snoozeWakeup() {
//       if(!prof_on) return; 
// dbg(DBG_USR1, "POWER: Mote %d SNOOZE END at %lld\n", NODE_NUM,
//           tos_state.tos_time);
     }
...
/********************** End State Transition Code  *******************/
     ...
     void print_power_info() {
	  ...
	  for(mote=0; mote < tos_state.num_nodes; mote++) {
	       ...
	       dbg(DBG_USR1,"POWER: Mote %d CPU_CYCLES %.1lf at %lld\n", mote,
