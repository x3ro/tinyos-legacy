...
TOS_TASK(check_X){
	...
	VAR(x_val) -= VAR(x_val) >> 3;
	VAR(x_val) += data;
	if(VAR(new_pot_x) == 1)  VAR(x_avg) = data << 2;
	VAR(new_pot_x) = 0;
	if(data > 500 || data < 400 || VAR(stable) > 100){
		if(VAR(off_range_count_x) > OFF_RANGE_MAX || 
			(VAR(stable) != 0 && VAR(off_range_count_x) != 0)){
			if(data > 500){
				if(VAR(potx) < 255)
					VAR(potx) ++;
			}else{
...
TOS_TASK(check_Y){
	...
	VAR(y_val) -= VAR(y_val) >> 3;
	VAR(y_val) += data;
	
	if(VAR(new_pot_y) == 1)  VAR(y_avg) = data << 2;
	VAR(new_pot_y) = 0;

	//POT adjustment step.
	if(data > 500 || data < 400 || VAR(stable) > 100){
		if(VAR(off_range_count_y) > OFF_RANGE_MAX || 
			(VAR(stable) != 0 && VAR(off_range_count_y) != 0)){
			if(data > 500){
	...
	if(VAR(mag_sent_ctr) > 0){
	  ...
	  //magnitude = VAR(x_val) >> 5;
	  //magnitude |= ((VAR(y_val) << 3) & 0xff00);
