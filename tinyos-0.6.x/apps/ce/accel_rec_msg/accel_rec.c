...
#define EPROM_SIZE   32768    // Eprom size in bytes                               11/03
#define EPROM_WSAMPLES 4  //# of samples to buffer before writing to EPROM
                          // i.e. write 16 bytes at a time                          11/03
#define EPROM_LAST_WRITE_ADDR   (EPROM_SIZE/(EPROM_WSAMPLES *4)) -1   //last eprom write address      11/03
...
char TOS_EVENT(ACCEL_REC_WRITE_LOG_DONE)(char success){
  ...
  if (VAR(eeprom_wline) > EPROM_LAST_WRITE_ADDR){
...
//xmit status
   if((msgptr->data[4] == CMD_STATUS ) && (msgptr->addr == TOS_LOCAL_ADDRESS )){
          ...
          VAR(msg_pending) = 1;  
		  VAR(msg_pending) = TOS_CALL_COMMAND(ACCEL_REC_SEND_MSG)(TOS_UART_ADDR, BASE_STATION_REQ, VAR(ptrmsg));
...
//         = 1         : dump entire eprom, 1st to last address
...
//     [9]   = hi byte
  if((msgptr->data[4] == CMD_START_READ) && (msgptr->addr == TOS_LOCAL_ADDRESS)){
      ...
      if (msgptr->data[5] == 0){              //read all of EPROM
        ...
        VAR(read_line) = 0;
        VAR(eeprom_rline) = EPROM_READ_SIZE;
...
//============================= BROADCAST MESSAGES ============================          
  if(msgptr->addr == TOS_BCAST_ADDR){  
	...
	if (msgptr->data[0] == VAR(seq_old)) return(msgptr);
...
//     VAR(acount) = 0;   
     ...
     TOS_CALL_COMMAND(LOGGER_APPEND_INIT)();
