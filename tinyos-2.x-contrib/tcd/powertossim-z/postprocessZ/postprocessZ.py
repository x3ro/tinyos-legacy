...
eff_table_filename = "EfficiencyTable.csv" # table of battery efficiency vs current load
rec_table_filename = "RecoveryProbabilityTable.csv" # table of recovery probability vs battery remaining capacity
...
class EfficiencyTable:
    ...
    def __init__(self, eff_table_filename):
        ...
        for row in reader:
            ...
            i+=1
        self.tableSize=i
        print 'Efficiency table loaded, size=' + str(self.tableSize)
...
class RecoveryTable:
    ...
    def __init__(self, rec_table_filename):
        ...
        for row in reader:
            ...
            i+=1
        self.tableSize=i
        print 'Recovery table loaded, size=' + str(self.tableSize)
...
def handle_event(l):
    ...
    if (time>maxtime):
        maxtime=time
    if debug: print "mote: %d, time: %.0f" % (mote,time)
...
def print_summary():
    ...
    for mote in range(maxseen+1):
        ...
        if prettyprint:
            ...
            print "Mote %d, Battery mAh remaining: %.1f" %(mote, battery[mote])
            if mote_died[mote]>0:
