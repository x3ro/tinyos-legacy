...
char TOS_COMMAND(LOGGER_TEST_INIT)(){
  ...
  TOS_CALL_COMMAND(LOGGER_CLOCK_INIT)(255, 3);
  TOS_CALL_COMMAND(LOGGER_ADC_INIT)();
  VAR(entries) = (log_entry*)VAR(msg).data;
  printf("LOGGER_TEST initialized\n");
	VAR(read_msg_ptr) = 0;
        TOS_CALL_COMMAND(LOGGER_TEST_READ_LOG)(VAR(time),VAR(read_msg)[VAR(read_msg_ptr)].data);
	VAR(read_msg_ptr) ^= 1;
  return 1;
}
...
TOS_MsgPtr TOS_EVENT(LOGGER_TEST_READ_MSG)(TOS_MsgPtr msg){
	...
	log_line |= data[0] << 8;
    	...
    	TOS_CALL_COMMAND(GREEN_LED_TOGGLE)();
        TOS_CALL_COMMAND(LOGGER_TEST_READ_LOG)(log_line,VAR(read_msg)[0].data);
        return msg;
}
char TOS_EVENT(LOGGER_TEST_MSG_SENT)(TOS_MsgPtr msg){
	...
	//if((msg == &(VAR(read_msg)[VAR(read_msg_ptr)]))) {
		VAR(read_send_pending) = 0;
		VAR(time)++;
        	TOS_CALL_COMMAND(LOGGER_TEST_READ_LOG)(VAR(time),VAR(read_msg)[VAR(read_msg_ptr)].data);
		VAR(read_msg_ptr) ^= 1;
		TOS_CALL_COMMAND(COMM_SEND_MSG)(0xff,0x06,&(VAR(read_msg)[VAR(read_msg_ptr)]));
...
char TOS_EVENT(LOGGER_TEST_READ_LOG_DONE)(char* data, char success){
	...
	if(VAR(time) == 0){
		VAR(time) ++;
        	TOS_CALL_COMMAND(LOGGER_TEST_READ_LOG)(VAR(time),VAR(read_msg)[1].data);
		TOS_CALL_COMMAND(COMM_SEND_MSG)(0xff,0x06,&(VAR(read_msg)[0]));
	}
	return 1;
}
...
char TOS_EVENT(LOGGER_DATA_READY)(int data){
	...
	VAR(entries)->records[(int)VAR(count)].val = VAR(reading);
